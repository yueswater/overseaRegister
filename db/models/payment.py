...
from db.config.payment_schema import PAYMENT_METADATA, ALL_ITEMS

@dataclass
class Payment:
    items: Dict[str, int] = field(default_factory=lambda: {
    ...
    def from_dict(cls, data: Dict[str, int]):
        items = {
            item: data.get(item, PAYMENT_METADATA[item]["amount"])
